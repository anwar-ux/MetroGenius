...
class JobApplicationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<EmployeeJobApplicationBloc,
      ...
      listener: (context, state) {
        if (state.status == FormStatus.success) {
          showCustomSnackbar(
              context,
              'Success',
              'Application submitted successfully, We will contact you soon ${state.name}',
              Colors.green);
      ...
      builder: (context, state) {
         if (state.status == FormStatus.success) {
          return Center(child: Text('Application submitted successfully!'));
        } else if (state.status == FormStatus.error) {
          ...
          return Scaffold(
              ...
              appBar: AppBar(
                ...
                titleTextStyle: GoogleFonts.urbanist(
                    color: AppColors.seconderyColor, fontSize: 22),
                centerTitle: true,
              ),
              body: Stack(
                children: [
                                  ...
                                  CustomButton(
                                      ...
                                      action: () async {
                                        if (_formKey.currentState!.validate()) {
                                          ...
                                          if (img != null) {
                                            try {
                                              photoUrl = await ImageConvertion
